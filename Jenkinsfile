pipeline {
    ...
    stages {
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag githubpf yash5090/githubpf:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/githubpf:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/githubpf:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/githubpf:latest'
                       sh 'docker-scout cves yash5090/githubpf:latest'
                       sh 'docker-scout recommendations yash5090/githubpf:latest'
                       sh 'docker-scout attestation yash5090/githubpf:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/githubpf:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Docker Scout Artifacts Analysis') {
            steps {
                script{
                  withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                      sh 'docker-scout sbom --format list yash5090/githubpf:latest' // docker-scout sbom yash5090/githubpf:latest
                    }
                }   
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop githubpf | true"
                sh "docker rm githubpf | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name githubpf -p 3000:3000 yash5090/githubpf:latest' 
                //}
            }
        }
